...
const equipmentEntrySchema = zod_1.z.object({
    name: zod_1.z.string().min(1).max(255),
    type: zod_1.z.enum(['OWNED', 'RENTED']).default('RENTED'),
    hoursUsed: zod_1.z.coerce.number().min(0).optional(),
    rentalCostPerHour: zod_1.z.coerce.number().min(0).optional(),
    totalCost: zod_1.z.coerce.number().min(0).optional(),
    note: zod_1.z.string().optional(),
});
const labourEntrySchema = zod_1.z.object({
    workerName: zod_1.z.string().min(1).max(255),
    role: zod_1.z.string().min(1).max(100).default('Worker'),
    hoursWorked: zod_1.z.coerce.number().min(0).optional(),
    dailyWage: zod_1.z.coerce.number().min(0).optional(),
    overtime: zod_1.z.coerce.number().min(0).optional(),
    overtimeRate: zod_1.z.coerce.number().min(0).optional(),
    note: zod_1.z.string().optional(),
});
async function mobileRoutes(app) {
    ...
    app.get('/sync', async (request) => {
        ...
        if (!worker)
            return { worker: null, assignedSites: [], company: null, pendingNotifications: [] };
        return {
    ...
    app.post('/submissions', async (request, reply) => {
        ...
        const schema = zod_1.z.object({
            ...
            workTypeId: zod_1.z.string().uuid(),
            zoneId: zod_1.z.string().uuid().optional(),
            note: zod_1.z.string().optional(),
            latitude: zod_1.z.coerce.number(),
            longitude: zod_1.z.coerce.number(),
            gpsAccuracy: zod_1.z.coerce.number().optional(),
            deviceTimestamp: zod_1.z.string(),
            deviceId: zod_1.z.string().optional(),
            isBeforePhoto: zod_1.z.coerce.boolean().default(false),
            linkedSubmissionId: zod_1.z.string().uuid().optional(),
            customData: zod_1.z.string().optional(),
            photoStepMeta: zod_1.z.string().optional(),
        });
        ...
        if (!assignment)
            return reply.code(403).send({ error: 'You are not assigned to this site' });
        // Validate zone belongs to site
        if (data.zoneId) {
            const zone = await app.prisma.zone.findFirst({ where: { id: data.zoneId, siteId: data.siteId } });
            if (!zone)
                return reply.code(400).send({ error: 'Zone does not belong to this site' });
        }
        ...
        if (!siteWorkType)
            return reply.code(400).send({ error: 'Work type is not enabled for this site' });
        let distanceFromSite = null;
        let isWithinRadius = true;
        if (site.latitude && site.longitude) {
            distanceFromSite = (0, geo_js_1.haversineDistance)(data.latitude, data.longitude, site.latitude, site.longitude);
        ...
        const photoStepsDef = workType?.photoSteps || [];
        ...
        const submission = await app.prisma.submission.create({
            data: {
                ...
                customData: parsedCustomData ?? undefined,
                photos: {
                    create: photoRecords,
                },
            },
    ...
    app.post('/submissions/batch', async (request, reply) => {
        const schema = zod_1.z.object({
            submissions: zod_1.z.array(zod_1.z.object({
                ...
                gpsAccuracy: zod_1.z.number().optional(),
                deviceTimestamp: zod_1.z.string(),
                deviceId: zod_1.z.string().optional(),
                isBeforePhoto: zod_1.z.boolean().default(false),
        ...
        for (const sub of submissions) {
            try {
                ...
                if (!site) {
                    errors.push(`Site ${sub.siteId} not found`);
                    continue;
                }
                // Verify worker is assigned to this site
                const assignment = await app.prisma.siteWorker.findUnique({
                ...
                if (!assignment) {
                    errors.push(`Not assigned to site ${sub.siteId}`);
                    continue;
                }
                // Validate zone belongs to site
                if (sub.zoneId) {
                    ...
                    if (!zone) {
                        errors.push(`Zone does not belong to site`);
                        continue;
                    }
                }
                // Validate work type is enabled for site
                ...
                if (!siteWorkType) {
                    errors.push(`Work type not enabled for site`);
                    continue;
                }
                let distanceFromSite = null;
                let isWithinRadius = true;
                if (site.latitude && site.longitude) {
                    distanceFromSite = (0, geo_js_1.haversineDistance)(sub.latitude, sub.longitude, site.latitude, site.longitude);
                    isWithinRadius = distanceFromSite <= site.gpsRadiusMetres;
                }
                ...
                if (existing) {
                    errors.push(`Duplicate submission skipped`);
                    continue;
                }
                const photoRecords = [];
    ...
    app.post('/issues', async (request, reply) => {
        ...
        const schema = zod_1.z.object({
            ...
            severity: zod_1.z.enum(['LOW', 'MEDIUM', 'HIGH', 'URGENT']).default('MEDIUM'),
    ...
    app.post('/daily-log', async (request, reply) => {
        ...
        const { siteId } = zod_1.z.object({ siteId: zod_1.z.string().uuid() }).parse(request.body);
        ...
        if (!assignment)
            return reply.code(403).send({ error: 'Not assigned to this site' });
    ...
    app.post('/daily-log/:logId/photo', async (request, reply) => {
        ...
        if (!photoBuffer)
            return reply.code(400).send({ error: 'No photo provided' });
