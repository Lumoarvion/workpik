...
export async function dashboardRoutes(app: FastifyInstance) {
  ...
  app.get('/', async (request) => {
    ...
    const siteActiveWorkers = new Map<string, number>();
  ...
  app.get('/trade-progress', async (request) => {
    ...
    const siteWhere: any = { companyId, status: 'ACTIVE' };
    if (request.user!.role === 'MANAGER') siteWhere.managers = { some: { userId: request.user!.id } };
    if (request.user!.role === 'CLIENT') siteWhere.clients = { some: { userId: request.user!.id } };
  ...
  app.get('/site/:siteId', async (request, reply) => {
    ...
    const siteWhere: any = { id: siteId, companyId: request.user!.companyId };
