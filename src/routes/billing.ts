...
const equipmentSchema = z.object({
  ...
  rentalCostPerHour: z.number().min(0).optional(),
...
export async function billingRoutes(app: FastifyInstance) {
  ...
  async function verifySite(siteId: string, companyId: string) {
  ...
  app.get('/sites/:siteId/billing', async (request, reply) => {
    ...
    if (query.dateFrom || query.dateTo) {
      where.logDate = {};
      if (query.dateFrom) where.logDate.gte = new Date(query.dateFrom);
      if (query.dateTo) where.logDate.lte = new Date(query.dateTo);
    }
    ...
    return paginatedResponse(processedData, total, query);
  });
  ...
  app.get('/sites/:siteId/billing/:logId', async (request, reply) => {
    const { siteId, logId } = request.params as { siteId: string; logId: string };
    ...
    for (const eq of log.equipmentLogs) {
      for (const p of eq.billingPhotos) {
        if (p.photoUrl) p.photoUrl = await app.s3.getSignedUrl(p.photoUrl);
        if (p.thumbnailUrl) p.thumbnailUrl = await app.s3.getSignedUrl(p.thumbnailUrl);
      }
    }
  ...
  app.put('/sites/:siteId/billing/:logId', async (request, reply) => {
    ...
    if (log.count === 0) return reply.code(404).send({ error: 'Daily log not found' });
  ...
  app.delete('/sites/:siteId/billing/:logId', async (request, reply) => {
    ...
    if (result.count === 0) return reply.code(404).send({ error: 'Daily log not found' });
  ...
  app.get('/sites/:siteId/billing/:logId/trade-summary', async (request, reply) => {
    ...
    const tradeMap = new Map<string, { totalQty: number; count: number; unit: string; workTypes: Set<string> }>();
    for (const sub of submissions) {
      const trade = sub.workType.trade || 'General';
      if (!tradeMap.has(trade)) tradeMap.set(trade, { totalQty: 0, count: 0, unit: sub.workType.billingUnit || '', workTypes: new Set() });
      const entry = tradeMap.get(trade)!;
      entry.count++;
      entry.workTypes.add(sub.workType.name);
      const cd = sub.customData as any;
      if (cd?.quantity && typeof cd.quantity === 'number') entry.totalQty += cd.quantity;
    }
  ...
  app.put('/sites/:siteId/billing/:logId/equipment/:equipId', async (request, reply) => {
    const { equipId } = request.params as { siteId: string; logId: string; equipId: string };
  ...
  app.put('/sites/:siteId/billing/:logId/materials/:materialId', async (request, reply) => {
    const { materialId } = request.params as { siteId: string; logId: string; materialId: string };
  ...
  app.put('/sites/:siteId/billing/:logId/labour/:labourId', async (request, reply) => {
    const { labourId } = request.params as { siteId: string; logId: string; labourId: string };
  ...
  app.patch('/sites/:siteId/billing/:logId/labour/:labourId/paid', async (request, reply) => {
    const { siteId, labourId } = request.params as { siteId: string; logId: string; labourId: string };
  ...
  app.get('/sites/:siteId/billing-payroll', async (request, reply) => {
    ...
    const logWhere: any = { siteId };
    if (query.dateFrom || query.dateTo) {
      logWhere.logDate = {};
      if (query.dateFrom) logWhere.logDate.gte = new Date(query.dateFrom);
      if (query.dateTo) logWhere.logDate.lte = new Date(query.dateTo);
    }
    ...
    for (const l of labourLogs) {
      ...
      try {
        if (l.note) {
          ...
          if (parsed?.count && typeof parsed.count === 'number') count = parsed.count;
        }
      ...
      if (l.paid) r.paidAmount += amount;
      else r.unpaidAmount += amount;
    }
