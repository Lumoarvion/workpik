...
const listSubmissionsQuery = paginationSchema.extend({
  ...
  status: z.enum(['PENDING_SYNC', 'SUBMITTED', 'FLAGGED', 'RETAKE_REQUESTED']).optional(),
...
export async function submissionRoutes(app: FastifyInstance) {
  ...
  app.get('/share/:shareToken', async (request, reply) => {
    ...
    for (const photo of link.submission.photos) {
      if (photo.photoUrl) photo.photoUrl = await app.s3.getSignedUrl(photo.photoUrl);
      if (photo.thumbnailUrl) photo.thumbnailUrl = await app.s3.getSignedUrl(photo.thumbnailUrl);
    }
  ...
  app.get('/stats', async (request) => {
    ...
    const where: any = { site: { companyId: request.user!.companyId } };
  ...
  app.get('/:id', async (request, reply) => {
    ...
    let flaggedByName: string | null = null;
    if (sub.flaggedBy) {
  ...
  app.put('/:id/flag', async (request, reply) => {
    ...
    if (result.count === 0) return reply.code(404).send({ error: 'Submission not found' });
